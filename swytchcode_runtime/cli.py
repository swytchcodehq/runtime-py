...
def run_cli(args: list[str], *, cwd: str | None = None, env: dict | None = None) -> Any:
    ...
    try:
        r = subprocess.run(cmd, capture_output=True, cwd=cwd or os.getcwd(), env=run_env)
    except FileNotFoundError as e:
        raise SwytchcodeError("Failed to spawn swytchcode; is the CLI installed?", e) from e
        
    if r.returncode != 0:
        raise SwytchcodeError(r.stderr.decode("utf-8", "replace").strip() or "command failed", r.returncode)
